...
while xcount < numcols:
    ...
    while ycount < numrows:
        ...
        if value == 0:
            ...
            worksheet.write(ycount+excel_offset_row,xcount+excel_offset_col,value,money_format)
        ycount += 1
    xcount += 1
...
for j in excel_matrix[0]:
    if type(j)!=int and today >= j and today < j + timedelta(days=7):
        #print excel_matrix.index(j)
        #print type(oppID)
        today_col = excel_matrix[0].index(j) + excel_offset_col
        print today_col
    else:
        pass    
...
for i in range(0,numcols+2):
    ...
    col += 1
    formula = '=SUM('+col_reference_row2(col_ref2,excel_offset_row+2,excel_offset_row+numrows)+')'
    worksheet.write_formula(excel_offset_row+numrows,col-2,formula,money_format)


#function collapses the columns for dates earlier that one month ago. 
collapse_column(excel_offset_col+2,today_col-4,worksheet)
oppID_col = colnum_string(excel_offset_col+1)+':'+colnum_string(excel_offset_col+1)
worksheet.set_column(oppID_col, 20)

collapse_column(excel_offset_col+2,today_col-4,worksheet)
collapse_column(today_col+8,numcols+excel_offset_col,worksheet)
col_ref = col_reference(today_col-4,numcols+10)
